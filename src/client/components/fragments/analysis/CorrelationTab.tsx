...
import { correlationPlotFetcher, mostCorrelatedTableFetcher } from "@/fetchers";
import { MostCorrelatedTable, MostCorrelatedTableRow, PlotlyResponseData } from "@/types";
import { CorrelationPlotRequest, MostCorrelatedAnalysisRequest } from "@/types/requests";
import { CORRELATION_MEASURE_OPTIONS, CorrelationMeasure, GENE_TYPE_OPTIONS, GeneTypeEnum } from "@/constants";
...
export default function CorrelationTab({ fragment, expressedIn }: EnrichmentAnalysisTabProps) {
    ...
    const [state, dispatch] = useBasicReducer<MostCorrelatedAnalysisRequest>({
        type: GeneTypeEnum.GENES,
        measure: CorrelationMeasure.PEARSON,
    });
    const onRestore = useCallback(
        ({ dataset, ...params }: MostCorrelatedAnalysisRequest) => {
            setDataset(dataset);
            dispatch({ type: "many", payload: params });
        },
        [dispatch, setDataset],
    );
    const correlationTableAnalysis = useRemoteAnalysis<MostCorrelatedAnalysisRequest, MostCorrelatedTable>(
        mostCorrelatedTableFetcher,
        "ct-",
        true,
        "Your correlation analysis has been completed!",
        onRestore,
    );
    const correlationPlotAnalysis = useRemoteAnalysis<CorrelationPlotRequest, PlotlyResponseData>(
        correlationPlotFetcher,
        false,
        false,
    );
